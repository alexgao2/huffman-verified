// Dafny program project.dfy compiled into C#
...
// Command-line arguments: test --no-verify /home/guransh/Desktop/ECS/huffman-verified/project.dfy
// project.dfy

method {:verify false} {:main} _Test__Main_(_noArgsParameter: seq<seq<char>>)
{
  var success: bool := true;
  print @""Huffman.DemoRun: "";
    [[ try { ]]
{
      DemoRun()
      {
...
module Huffman {
  function Weight<Sym(==)>(t: HTree<Sym>): nat
    decreases t
  {
    t.w
  }
  ...
  function PrefixMap<Sym(==)>(b: bool, m: map<Sym, seq<bool>>): map<Sym, seq<bool>>
    decreases b, m
  {
    map s: Sym {:trigger m[s]} {:trigger s in m.Keys} | s in m.Keys :: s := [b] + m[s]
  }
  ...
  lemma /*{:_inductionTrigger Leaves(t)}*/ /*{:_inductionTrigger Codes(t)}*/ /*{:_inductionTrigger ValidTree(t)}*/ /*{:_induction t}*/ CodesKeys<Sym>(t: HTree<Sym>)
    requires ValidTree(t)
    ensures Codes(t).Keys == Leaves(t)
    decreases t
  {
    match t
    case {:split false} Leaf(s, _ /* _v5 */) =>
    case {:split false} Node(_ /* _v6 */, l, r) =>
      CodesKeys(l);
      CodesKeys(r);
  ...
  lemma /*{:_inductionTrigger Codes(t)}*/ /*{:_inductionTrigger Leaves(t)}*/ /*{:_inductionTrigger NonTrivialTree(t)}*/ /*{:_inductionTrigger ValidTree(t)}*/ /*{:_induction t}*/ CodesNonEmpty<Sym>(t: HTree<Sym>)
    requires ValidTree(t) && NonTrivialTree(t)
    ensures forall s: Sym {:trigger s in Codes(t).Keys} {:trigger s in Leaves(t)} :: s in Leaves(t) ==> s in Codes(t).Keys
    ensures forall s: Sym {:trigger Codes(t)[s]} {:trigger s in Leaves(t)} :: s in Leaves(t) ==> |Codes(t)[s]| > 0
    decreases t
  {
    CodesKeys(t);
    assert forall s: Sym {:trigger s in Codes(t).Keys} {:trigger s in Leaves(t)} :: s in Leaves(t) ==> s in Codes(t).Keys;
    match t
    case {:split false} Leaf(_ /* _v7 */, _ /* _v8 */) =>
      assert false;
    case {:split false} Node(_ /* _v9 */, l, r) =>
      CodesKeys(l);
      CodesKeys(r);
      PrefixMapKeys(false, Codes(l));
      PrefixMapKeys(true, Codes(r));
  ...
  predicate PrefixFreeMap<Sym>(m: map<Sym, seq<bool>>)
    decreases m
  {
    forall s: Sym, t: Sym {:trigger m[t], m[s]} {:trigger m[t], s in m.Keys} {:trigger m[s], t in m.Keys} {:trigger t in m.Keys, s in m.Keys} | s in m.Keys && t in m.Keys && s != t :: 
      !IsPrefix(m[s], m[t]) &&
      !IsPrefix(m[t], m[s])
  }
  ...
  lemma PrefixMapPreservesPrefixFree<Sym>(b: bool, m: map<Sym, seq<bool>>)
    ...
    decreases b, m
  {
    forall s: Sym, t: Sym | s in PrefixMap(b, m) && t in PrefixMap(b, m) && s != t
      ensures !IsPrefix(PrefixMap(b, m)[s], PrefixMap(b, m)[t]) && !IsPrefix(PrefixMap(b, m)[t], PrefixMap(b, m)[s])
    {
      ghost var cs := PrefixMap(b, m)[s];
      ghost var ct := PrefixMap(b, m)[t];
      assert s in m.Keys;
      assert t in m.Keys;
  ...
  lemma /*{:_inductionTrigger Codes(t)}*/ /*{:_inductionTrigger ValidTree(t)}*/ /*{:_induction t}*/ CodesPrefixFree<Sym>(t: HTree<Sym>)
    requires ValidTree(t)
    ensures PrefixFreeMap(Codes(t))
    decreases t
  {
    match t
    case {:split false} Leaf(s, _ /* _v10 */) =>
    case {:split false} Node(_ /* _v11 */, l, r) =>
      CodesPrefixFree(l);
      CodesPrefixFree(r);
      ...
      assert Leaves(l) !! Leaves(r) by {
        match t
        case {:split false} Node(_ /* _v12 */, l, r) =>
          assert Leaves(l) !! Leaves(r);
      }
      assert PrefixMap(false, Codes(l)).Keys !! PrefixMap(true, Codes(r)).Keys;
      forall s: Sym, t': Sym | s in Codes(t) && t' in Codes(t) && s != t'
        ensures !IsPrefix(Codes(t)[s], Codes(t)[t']) && !IsPrefix(Codes(t)[t'], Codes(t)[s])
      {
        CodesKeys(t);
        ...
        if s in Leaves(l) && t' in Leaves(l) {
          ...
          if s in Leaves(l) {
            ...
            MapUnionLookupLeft(PrefixMap(false, Codes(l)), PrefixMap(true, Codes(r)), s);
            MapUnionLookupRight(PrefixMap(false, Codes(l)), PrefixMap(true, Codes(r)), t');
            ghost var cs := Codes(t)[s];
            ghost var ct := Codes(t)[t'];
            PrefixMapLookup(false, Codes(l), s);
  ...
  function EncodeWithCodes<Sym(==)>(code: map<Sym, seq<bool>>, msg: seq<Sym>): seq<bool>
    requires forall s: Sym {:trigger s in code.Keys} {:trigger s in msg} :: s in msg ==> s in code.Keys
    decreases |msg|
  {
    if |msg| == 0 then
      []
    else
      code[msg[0]] + EncodeWithCodes(code, msg[1..])
  }

  function Encode<Sym(==)>(t: HTree<Sym>, msg: seq<Sym>): seq<bool>
    requires ValidTree(t)
    requires forall s: Sym {:trigger s in Codes(t).Keys} {:trigger s in msg} :: s in msg ==> s in Codes(t).Keys
    decreases |msg|
  {
    EncodeWithCodes(Codes(t), msg)
  }

  lemma /*{:_inductionTrigger Encode(t, msg)}*/ /*{:_inductionTrigger Codes(t)}*/ /*{:_inductionTrigger NonTrivialTree(t)}*/ /*{:_inductionTrigger ValidTree(t)}*/ /*{:_induction t}*/ EncodeLenLowerBound<Sym>(t: HTree<Sym>, msg: seq<Sym>)
    requires ValidTree(t) && NonTrivialTree(t)
    requires forall s: Sym {:trigger s in Codes(t).Keys} {:trigger s in msg} :: s in msg ==> s in Codes(t).Keys
  ...
  lemma /*{:_inductionTrigger [true] + bits, HTree<Sym>.Node(w, l, r)}*/ /*{:_inductionTrigger [false] + bits, HTree<Sym>.Node(w, l, r)}*/ /*{:_induction l, r, bits}*/ DecodeOneFromNodeStep<Sym>(w: nat, l: HTree<Sym>, r: HTree<Sym>, bits: seq<bool>)
    ...
    decreases node, |bits|
  {
    ...
    case Leaf(s, _ /* _v13 */) =>
      Some((s, bits))
    case Node(_ /* _v14 */, l, r) =>
      if |bits| == 0 then
        None
      else if !bits[0] then
        DecodeOneFrom(l, bits[1..])
      else
        DecodeOneFrom(r, bits[1..])
  }
  ...
  lemma /*{:_inductionTrigger (s, suffix), Codes(t)}*/ /*{:_inductionTrigger (s, suffix), ValidTree(t)}*/ /*{:_induction t, suffix}*/ DecodeOneFromCorrect<Sym>(t: HTree<Sym>, s: Sym, suffix: seq<bool>)
    requires ValidTree(t)
  ...
  {
    match t
    case {:split false} Leaf(sym, _ /* _v15 */) =>
    ...
    case {:split false} Node(_ /* _v16 */, l, r) =>
      CodesKeys(l);
      CodesKeys(r);
      PrefixMapKeys(false, Codes(l));
      PrefixMapKeys(true, Codes(r));
    ...
    decreases fuel
  {
    if |bits| == 0 then
      ...
      match DecodeOneFrom(t, bits) case None() => None case Some(p) => match DecodeOptionFuel(t, p.1, fuel - 1) case None() => None case Some(restMsg) => Some([p.0] + restMsg)
  }
  ...
  lemma /*{:_inductionTrigger DecodeOptionFuel(t, Encode(t, msg), fuel)}*/ /*{:_induction t, fuel}*/ RoundTripFuel<Sym>(t: HTree<Sym>, msg: seq<Sym>, fuel: nat)
    requires ValidTree(t) && NonTrivialTree(t)
    requires forall s: Sym {:trigger s in Codes(t).Keys} {:trigger s in msg} :: s in msg ==> s in Codes(t).Keys
    requires fuel >= |msg|
    ...
    decreases |msg|
  {
    if |msg| == 0 {
      ...
      calc {
        ...
        match DecodeOneFrom(t, Codes(t)[msg[0]] + Encode(t, msg[1..])) { case None() => None case Some(p) => match DecodeOptionFuel(t, p.1, fuel - 1) { case None() => None case Some(restMsg) => Some([p.0] + restMsg) } };
        {
          assert DecodeOneFrom(t, Encode(t, msg)) == Some((msg[0], Encode(t, msg[1..])));
        }
        match Some((msg[0], Encode(t, msg[1..]))) { case None() => None case Some(p) => match DecodeOptionFuel(t, p.1, fuel - 1) { case None() => None case Some(restMsg) => Some([p.0] + restMsg) } };
        match DecodeOptionFuel(t, Encode(t, msg[1..]), fuel - 1) { case None() => None case Some(restMsg) => Some([msg[0]] + restMsg) };
        {
          assert DecodeOptionFuel(t, Encode(t, msg[1..]), fuel - 1) == Some(msg[1..]);
        }
        assert [msg[0]] + msg[1..] == msg; Some([msg[0]] + msg[1..]);
        Some(msg);
      }
    }
  }

  lemma /*{:_inductionTrigger Encode(t, msg)}*/ /*{:_inductionTrigger Codes(t)}*/ /*{:_inductionTrigger NonTrivialTree(t)}*/ /*{:_inductionTrigger ValidTree(t)}*/ /*{:_induction t}*/ DecodeEncodedRoundTrip<Sym>(t: HTree<Sym>, msg: seq<Sym>)
    requires ValidTree(t) && NonTrivialTree(t)
    requires forall s: Sym {:trigger s in Codes(t).Keys} {:trigger s in msg} :: s in msg ==> s in Codes(t).Keys
    ensures DecodeOption(t, Encode(t, msg)) == Some(msg)
    decreases t, msg
  {
    ghost var bits := Encode(t, msg);
    ...
    decreases |freq|
  {
    if |freq| == 0 then
      ...
      {freq[0].0} + Alphabet(freq[1..])
  }

  ghost predicate DistinctSyms<Sym>(freq: seq<(Sym, nat)>)
    decreases freq
  {
    forall i: int, j: int {:trigger freq[j], freq[i]} :: 
      0 <= i < j < |freq| ==>
        freq[i].0 != freq[j].0
  }
    ...
    decreases freq
  {
    |freq| >= 2 &&
    DistinctSyms(freq) &&
    forall i: int {:trigger freq[i]} :: 
      0 <= i < |freq| ==>
        freq[i].1 > 0
  }

  lemma /*{:_inductionTrigger |freq|}*/ /*{:_inductionTrigger Alphabet(freq)}*/ /*{:_induction freq}*/ InAlphabetImpliesExists<Sym>(freq: seq<(Sym, nat)>, s: Sym)
    requires s in Alphabet(freq)
    ensures exists i: int {:trigger freq[i]} :: 0 <= i < |freq| && freq[i].0 == s
    decreases |freq|
  {
    if |freq| == 0 {
      assert false;
    } else if s == freq[0].0 {
      assert exists i: int {:trigger freq[i]} :: 0 <= i < |freq| && freq[i].0 == s by {
        assert 0 < |freq| && freq[0].0 == s;
      }
    } else {
      assert s in Alphabet(freq[1..]);
      InAlphabetImpliesExists(freq[1..], s);
      ghost var i :| 0 <= i < |freq[1..]| && freq[1..][i].0 == s;
    ...
    decreases freq
  {
    if freq[0].0 in Alphabet(freq[1..]) {
      ...
      ghost var j :| 0 <= j < |freq[1..]| && freq[1..][j].0 == freq[0].0;
  ...
  lemma /*{:_inductionTrigger Alphabet(a) + Alphabet(b)}*/ /*{:_inductionTrigger a + b}*/ /*{:_induction a, b}*/ AlphabetConcat<Sym>(a: seq<(Sym, nat)>, b: seq<(Sym, nat)>)
  ...
  ghost predicate SortedByWeight<Sym>(q: seq<HTree<Sym>>)
    decreases q
  {
    forall i: int, _t#0: int {:trigger q[_t#0], q[i]} | _t#0 == i + 1 :: 
      0 <= i &&
      i < |q| - 1 ==>
        Weight(q[i]) <= Weight(q[_t#0])
  }
    ...
    decreases |q|
  {
    if |q| <= 1 then
      ...
      Leaves(q[0]) !! UnionLeaves(q[1..]) && DisjointQueue(q[1..])
  }

  ghost predicate QueueInv<Sym>(q: seq<HTree<Sym>>, alphabet: set<Sym>)
    decreases q, alphabet
  {
    |q| >= 1 &&
    SortedByWeight(q) &&
    DisjointQueue(q) &&
    ...
    decreases |q|
  {
    if |q| == 0 then
      [t]
    else if Weight(t) <= Weight(q[0]) then
      [t] + q
    else
      [q[0]] + InsertSorted(q[1..], t)
  }

  lemma /*{:_inductionTrigger InsertSorted(q, t)}*/ /*{:_induction q, t}*/ InsertSortedLength<Sym>(q: seq<HTree<Sym>>, t: HTree<Sym>)
    requires SortedByWeight(q)
  ...
  lemma /*{:_inductionTrigger InsertSorted(q, t)}*/ /*{:_induction q, t}*/ InsertSortedPreservesSorted<Sym>(q: seq<HTree<Sym>>, t: HTree<Sym>)
    requires SortedByWeight(q)
  ...
  lemma /*{:_inductionTrigger Leaves(t), SortedByWeight(q)}*/ /*{:_inductionTrigger InsertSorted(q, t)}*/ /*{:_induction q, t}*/ UnionLeavesInsertSorted<Sym>(q: seq<HTree<Sym>>, t: HTree<Sym>)
    requires SortedByWeight(q)
  ...
  lemma /*{:_inductionTrigger InsertSorted(q, t)}*/ /*{:_inductionTrigger UnionLeaves(q), Leaves(t)}*/ /*{:_inductionTrigger Leaves(t), SortedByWeight(q)}*/ /*{:_induction q, t}*/ DisjointQueueInsertSorted<Sym>(q: seq<HTree<Sym>>, t: HTree<Sym>)
    requires SortedByWeight(q)
    ...
    decreases |q|
  {
    if |q| == 0 {
      ...
      forall x: Sym | x in Leaves(q[0]) && x in Leaves(t)
        ensures false
      {
  ...
  lemma /*{:_inductionTrigger InsertSorted(q, t)}*/ /*{:_induction q, t}*/ InsertSortedPreservesValidity<Sym>(q: seq<HTree<Sym>>, t: HTree<Sym>)
    requires SortedByWeight(q)
    requires forall k: int {:trigger q[k]} :: 0 <= k < |q| ==> ValidTree(q[k])
    requires ValidTree(t)
    ensures forall k: int {:trigger InsertSorted(q, t)[k]} :: 0 <= k < |InsertSorted(q, t)| ==> ValidTree(InsertSorted(q, t)[k])
    decreases |q|
  {
    if |q| == 0 {
    } else if Weight(t) <= Weight(q[0]) {
      forall k: int | 0 <= k < |[t] + q|
      ...
      forall k: int | 0 <= k < |[q[0]] + InsertSorted(q[1..], t)|
  ...
  lemma /*{:_inductionTrigger |freq|}*/ /*{:_inductionTrigger Alphabet(freq)}*/ /*{:_inductionTrigger DistinctSyms(freq)}*/ /*{:_induction freq}*/ AlphabetSize<Sym>(freq: seq<(Sym, nat)>)
    ...
    decreases freq
  {
    ...
    while i < |freq|
      ...
      invariant forall k: int {:trigger q[k]} :: 0 <= k < |q| ==> ValidTree(q[k])
      decreases |freq| - i
    {
      var s := freq[i].0;
      var w := freq[i].1;
      ...
      assert leaf.sym !in UnionLeaves(q) by {
        if leaf.sym in UnionLeaves(q) {
          ...
          ghost var j :| 0 <= j < i && freq[..i][j].0 == leaf.sym;
  ...
  lemma /*{:_inductionTrigger Encode(t, msg)}*/ /*{:_inductionTrigger Codes(t)}*/ /*{:_inductionTrigger NonTrivialTree(t)}*/ /*{:_inductionTrigger ValidTree(t)}*/ /*{:_induction t}*/ HuffmanCorrectness<Sym>(t: HTree<Sym>, msg: seq<Sym>)
    requires ValidTree(t) && NonTrivialTree(t)
    requires forall s: Sym {:trigger s in Codes(t).Keys} {:trigger s in msg} :: s in msg ==> s in Codes(t).Keys
    ensures PrefixFreeMap(Codes(t))
    ensures DecodeOption(t, Encode(t, msg)) == Some(msg)
  ...
  method {:test} DemoRun()
  {
    ...
    assert forall s: char {:trigger s in Codes(t).Keys} {:trigger s in msg} :: s in msg ==> s in Codes(t).Keys by {
      forall s: char | s in msg
  ...
  datatype HTree<Sym(==)> = Leaf(sym: Sym, w: nat) | Node(w: nat, left: HTree<Sym>, right: HTree<Sym>)
}
")]
...
namespace Huffman {
  ...
  public partial class __default {
    public static BigInteger Weight<__Sym>(Huffman._IHTree<__Sym> t) {
      return (t).dtor_w;
    }
    public static Dafny.IMap<__Sym,Dafny.ISequence<bool>> PrefixMap<__Sym>(bool b, Dafny.IMap<__Sym,Dafny.ISequence<bool>> m)
    {
      return Dafny.Helpers.Id<Func<Dafny.IMap<__Sym,Dafny.ISequence<bool>>, bool, Dafny.IMap<__Sym,Dafny.ISequence<bool>>>>((_0_m, _1_b) => ((System.Func<Dafny.IMap<__Sym,Dafny.ISequence<bool>>>)(() => {
        var _coll0 = new System.Collections.Generic.List<Dafny.Pair<__Sym,Dafny.ISequence<bool>>>();
        foreach (__Sym _compr_0 in ((_0_m).Keys).Elements) {
          __Sym _2_s = (__Sym)_compr_0;
          if (((_0_m).Keys).Contains(_2_s)) {
            _coll0.Add(new Dafny.Pair<__Sym,Dafny.ISequence<bool>>(_2_s, Dafny.Sequence<bool>.Concat(Dafny.Sequence<bool>.FromElements(_1_b), Dafny.Map<__Sym, Dafny.ISequence<bool>>.Select(_0_m,_2_s))));
          }
        }
        return Dafny.Map<__Sym,Dafny.ISequence<bool>>.FromCollection(_coll0);
      }))())(m, b);
    }
    public static Dafny.IMap<__Sym,Dafny.ISequence<bool>> Codes<__Sym>(Huffman._IHTree<__Sym> t) {
      Huffman._IHTree<__Sym> _source0 = t;
      {
        if (_source0.is_Leaf) {
          __Sym _0_s = _source0.dtor_sym;
          return Dafny.Map<__Sym, Dafny.ISequence<bool>>.FromElements(new Dafny.Pair<__Sym, Dafny.ISequence<bool>>(_0_s, Dafny.Sequence<bool>.FromElements()));
        }
      }
      {
        Huffman._IHTree<__Sym> _1_l = _source0.dtor_left;
        Huffman._IHTree<__Sym> _2_r = _source0.dtor_right;
        return Dafny.Map<__Sym, Dafny.ISequence<bool>>.Merge(Huffman.__default.PrefixMap<__Sym>(false, Huffman.__default.Codes<__Sym>(_1_l)), Huffman.__default.PrefixMap<__Sym>(true, Huffman.__default.Codes<__Sym>(_2_r)));
      }
    }
    public static bool IsPrefix(Dafny.ISequence<bool> a, Dafny.ISequence<bool> b)
    {
      return ((new BigInteger((a).Count)) <= (new BigInteger((b).Count))) && (Dafny.Helpers.Id<Func<Dafny.ISequence<bool>, Dafny.ISequence<bool>, bool>>((_0_a, _1_b) => Dafny.Helpers.Quantifier<BigInteger>(Dafny.Helpers.IntegerRange(BigInteger.Zero, new BigInteger((_0_a).Count)), true, (((_forall_var_0) => {
        BigInteger _2_i = (BigInteger)_forall_var_0;
        return !(((_2_i).Sign != -1) && ((_2_i) < (new BigInteger((_0_a).Count)))) || (((_0_a).Select(_2_i)) == ((_1_b).Select(_2_i)));
      }))))(a, b));
    }
    public static bool PrefixFreeMap<__Sym>(Dafny.IMap<__Sym,Dafny.ISequence<bool>> m) {
      return Dafny.Helpers.Id<Func<Dafny.IMap<__Sym,Dafny.ISequence<bool>>, bool>>((_0_m) => Dafny.Helpers.Quantifier<__Sym>(((_0_m).Keys).Elements, true, (((_forall_var_0) => {
        __Sym _1_s = (__Sym)_forall_var_0;
        return Dafny.Helpers.Quantifier<__Sym>(((_0_m).Keys).Elements, true, (((_forall_var_1) => {
          __Sym _2_t = (__Sym)_forall_var_1;
          return !(((((_0_m).Keys).Contains(_1_s)) && (((_0_m).Keys).Contains(_2_t))) && (!object.Equals(_1_s, _2_t))) || ((!(Huffman.__default.IsPrefix(Dafny.Map<__Sym, Dafny.ISequence<bool>>.Select(_0_m,_1_s), Dafny.Map<__Sym, Dafny.ISequence<bool>>.Select(_0_m,_2_t)))) && (!(Huffman.__default.IsPrefix(Dafny.Map<__Sym, Dafny.ISequence<bool>>.Select(_0_m,_2_t), Dafny.Map<__Sym, Dafny.ISequence<bool>>.Select(_0_m,_1_s)))));
        })));
      }))))(m);
    }
    public static Dafny.ISequence<bool> EncodeWithCodes<__Sym>(Dafny.IMap<__Sym,Dafny.ISequence<bool>> code, Dafny.ISequence<__Sym> msg)
    {
      Dafny.ISequence<bool> _0___accumulator = Dafny.Sequence<bool>.FromElements();
    TAIL_CALL_START: ;
      if ((new BigInteger((msg).Count)).Sign == 0) {
        return Dafny.Sequence<bool>.Concat(_0___accumulator, Dafny.Sequence<bool>.FromElements());
      } else {
        _0___accumulator = Dafny.Sequence<bool>.Concat(_0___accumulator, Dafny.Map<__Sym, Dafny.ISequence<bool>>.Select(code,(msg).Select(BigInteger.Zero)));
        Dafny.IMap<__Sym,Dafny.ISequence<bool>> _in0 = code;
        Dafny.ISequence<__Sym> _in1 = (msg).Drop(BigInteger.One);
        code = _in0;
        msg = _in1;
        goto TAIL_CALL_START;
      }
    }
    public static Dafny.ISequence<bool> Encode<__Sym>(Huffman._IHTree<__Sym> t, Dafny.ISequence<__Sym> msg)
    {
      return Huffman.__default.EncodeWithCodes<__Sym>(Huffman.__default.Codes<__Sym>(t), msg);
    }
    public static Huffman._IOption<_System._ITuple2<__Sym, Dafny.ISequence<bool>>> DecodeOneFrom<__Sym>(Huffman._IHTree<__Sym> node, Dafny.ISequence<bool> bits)
    {
    TAIL_CALL_START: ;
      Huffman._IHTree<__Sym> _source0 = node;
      {
        if (_source0.is_Leaf) {
          __Sym _0_s = _source0.dtor_sym;
          return Huffman.Option<_System._ITuple2<__Sym, Dafny.ISequence<bool>>>.create_Some(_System.Tuple2<__Sym, Dafny.ISequence<bool>>.create(_0_s, bits));
        }
      }
      {
        Huffman._IHTree<__Sym> _1_l = _source0.dtor_left;
        ...
        if ((new BigInteger((bits).Count)).Sign == 0) {
          return Huffman.Option<_System._ITuple2<__Sym, Dafny.ISequence<bool>>>.create_None();
        } else if (!((bits).Select(BigInteger.Zero))) {
          Huffman._IHTree<__Sym> _in0 = _1_l;
          Dafny.ISequence<bool> _in1 = (bits).Drop(BigInteger.One);
          node = _in0;
          bits = _in1;
          goto TAIL_CALL_START;
        } else {
          Huffman._IHTree<__Sym> _in2 = _2_r;
          Dafny.ISequence<bool> _in3 = (bits).Drop(BigInteger.One);
          node = _in2;
          bits = _in3;
          goto TAIL_CALL_START;
        }
    ...
    public static Huffman._IOption<Dafny.ISequence<__Sym>> DecodeOptionFuel<__Sym>(Huffman._IHTree<__Sym> t, Dafny.ISequence<bool> bits, BigInteger fuel)
    {
      if ((new BigInteger((bits).Count)).Sign == 0) {
        return Huffman.Option<Dafny.ISequence<__Sym>>.create_Some(Dafny.Sequence<__Sym>.FromElements());
      } else if ((fuel).Sign == 0) {
        return Huffman.Option<Dafny.ISequence<__Sym>>.create_None();
      } else {
        Huffman._IOption<_System._ITuple2<__Sym, Dafny.ISequence<bool>>> _source0 = Huffman.__default.DecodeOneFrom<__Sym>(t, bits);
        {
          if (_source0.is_None) {
            return Huffman.Option<Dafny.ISequence<__Sym>>.create_None();
          }
        ...
        {
          _System._ITuple2<__Sym, Dafny.ISequence<bool>> _0_p = _source0.dtor_value;
          Huffman._IOption<Dafny.ISequence<__Sym>> _source1 = Huffman.__default.DecodeOptionFuel<__Sym>(t, (_0_p).dtor__1, (fuel) - (BigInteger.One));
          {
            if (_source1.is_None) {
              return Huffman.Option<Dafny.ISequence<__Sym>>.create_None();
            }
          ...
          {
            Dafny.ISequence<__Sym> _1_restMsg = _source1.dtor_value;
            return Huffman.Option<Dafny.ISequence<__Sym>>.create_Some(Dafny.Sequence<__Sym>.Concat(Dafny.Sequence<__Sym>.FromElements((_0_p).dtor__0), _1_restMsg));
          }
        }
      }
    }
    public static Huffman._IOption<Dafny.ISequence<__Sym>> DecodeOption<__Sym>(Huffman._IHTree<__Sym> t, Dafny.ISequence<bool> bits)
    {
      return Huffman.__default.DecodeOptionFuel<__Sym>(t, bits, new BigInteger((bits).Count));
    }
    public static Dafny.ISequence<Huffman._IHTree<__Sym>> InsertSorted<__Sym>(Dafny.ISequence<Huffman._IHTree<__Sym>> q, Huffman._IHTree<__Sym> t)
    {
      Dafny.ISequence<Huffman._IHTree<__Sym>> _0___accumulator = Dafny.Sequence<Huffman._IHTree<__Sym>>.FromElements();
    TAIL_CALL_START: ;
      if ((new BigInteger((q).Count)).Sign == 0) {
        return Dafny.Sequence<Huffman._IHTree<__Sym>>.Concat(_0___accumulator, Dafny.Sequence<Huffman._IHTree<__Sym>>.FromElements(t));
      } else if ((Huffman.__default.Weight<__Sym>(t)) <= (Huffman.__default.Weight<__Sym>((q).Select(BigInteger.Zero)))) {
        return Dafny.Sequence<Huffman._IHTree<__Sym>>.Concat(_0___accumulator, Dafny.Sequence<Huffman._IHTree<__Sym>>.Concat(Dafny.Sequence<Huffman._IHTree<__Sym>>.FromElements(t), q));
      } else {
        _0___accumulator = Dafny.Sequence<Huffman._IHTree<__Sym>>.Concat(_0___accumulator, Dafny.Sequence<Huffman._IHTree<__Sym>>.FromElements((q).Select(BigInteger.Zero)));
        Dafny.ISequence<Huffman._IHTree<__Sym>> _in0 = (q).Drop(BigInteger.One);
        Huffman._IHTree<__Sym> _in1 = t;
        q = _in0;
        t = _in1;
        goto TAIL_CALL_START;
      }
    }
    public static Huffman._IHTree<__Sym> BuildHuffman<__Sym>(Dafny.ISequence<_System._ITuple2<__Sym, BigInteger>> freq)
    {
      Huffman._IHTree<__Sym> t = default(Huffman._IHTree<__Sym>);
      Dafny.ISequence<Huffman._IHTree<__Sym>> _0_q;
      _0_q = Dafny.Sequence<Huffman._IHTree<__Sym>>.FromElements();
      BigInteger _1_i;
      _1_i = BigInteger.Zero;
      while ((_1_i) < (new BigInteger((freq).Count))) {
        __Sym _2_s;
        _2_s = ((freq).Select(_1_i)).dtor__0;
        BigInteger _3_w;
        _3_w = ((freq).Select(_1_i)).dtor__1;
        Huffman._IHTree<__Sym> _4_leaf;
        _4_leaf = Huffman.HTree<__Sym>.create_Leaf(_2_s, _3_w);
        Dafny.ISequence<Huffman._IHTree<__Sym>> _5_oldq;
        _5_oldq = _0_q;
        _0_q = Huffman.__default.InsertSorted<__Sym>(_5_oldq, _4_leaf);
        _1_i = (_1_i) + (BigInteger.One);
      }
      while ((new BigInteger((_0_q).Count)) > (BigInteger.One)) {
        Dafny.ISequence<Huffman._IHTree<__Sym>> _6_oldq;
        _6_oldq = _0_q;
        Huffman._IHTree<__Sym> _7_a;
        _7_a = (_6_oldq).Select(BigInteger.Zero);
        Huffman._IHTree<__Sym> _8_b;
        _8_b = (_6_oldq).Select(BigInteger.One);
        Dafny.ISequence<Huffman._IHTree<__Sym>> _9_rest;
        _9_rest = (_6_oldq).Drop(new BigInteger(2));
        Huffman._IHTree<__Sym> _10_merged;
        _10_merged = Huffman.HTree<__Sym>.create_Node((Huffman.__default.Weight<__Sym>(_7_a)) + (Huffman.__default.Weight<__Sym>(_8_b)), _7_a, _8_b);
        Dafny.ISequence<Huffman._IHTree<__Sym>> _11_oldrest;
        _11_oldrest = _9_rest;
        _0_q = Huffman.__default.InsertSorted<__Sym>(_11_oldrest, _10_merged);
      }
      t = (_0_q).Select(BigInteger.Zero);
      return t;
    }
    public static void DemoRun()
    {
      Dafny.ISequence<_System._ITuple2<Dafny.Rune, BigInteger>> _0_freq;
      _0_freq = Dafny.Sequence<_System._ITuple2<Dafny.Rune, BigInteger>>.FromElements(_System.Tuple2<Dafny.Rune, BigInteger>.create(new Dafny.Rune('*'), new BigInteger(3)), _System.Tuple2<Dafny.Rune, BigInteger>.create(new Dafny.Rune('C'), new BigInteger(6)), _System.Tuple2<Dafny.Rune, BigInteger>.create(new Dafny.Rune('D'), new BigInteger(4)), _System.Tuple2<Dafny.Rune, BigInteger>.create(new Dafny.Rune('A'), new BigInteger(5)), _System.Tuple2<Dafny.Rune, BigInteger>.create(new Dafny.Rune('E'), new BigInteger(2)));
      Huffman._IHTree<Dafny.Rune> _1_t;
      Huffman._IHTree<Dafny.Rune> _out0;
      _out0 = Huffman.__default.BuildHuffman<Dafny.Rune>(_0_freq);
      _1_t = _out0;
      Dafny.ISequence<Dafny.Rune> _2_msg;
      _2_msg = Dafny.Sequence<Dafny.Rune>.FromElements(new Dafny.Rune('*'), new Dafny.Rune('C'), new Dafny.Rune('C'), new Dafny.Rune('D'), new Dafny.Rune('A'), new Dafny.Rune('C'), new Dafny.Rune('C'), new Dafny.Rune('*'), new Dafny.Rune('D'), new Dafny.Rune('A'), new Dafny.Rune('*'), new Dafny.Rune('C'), new Dafny.Rune('C'), new Dafny.Rune('D'), new Dafny.Rune('E'), new Dafny.Rune('A'), new Dafny.Rune('A'), new Dafny.Rune('E'), new Dafny.Rune('D'), new Dafny.Rune('A'));
      Dafny.ISequence<bool> _3_bits;
      _3_bits = Huffman.__default.Encode<Dafny.Rune>(_1_t, _2_msg);
      Huffman._IOption<Dafny.ISequence<Dafny.Rune>> _4_decoded;
      _4_decoded = Huffman.__default.DecodeOption<Dafny.Rune>(_1_t, _3_bits);
      Dafny.Helpers.Print((Dafny.Sequence<Dafny.Rune>.UnicodeFromString("Decoded: ")).ToVerbatimString(false));
      Dafny.Helpers.Print((_4_decoded));
  ...
  public abstract class Option<T> : _IOption<T> {
    ...
    public static Huffman._IOption<T> Default() {
      return create_None();
    }
    public static Dafny.TypeDescriptor<Huffman._IOption<T>> _TypeDescriptor() {
      return new Dafny.TypeDescriptor<Huffman._IOption<T>>(Huffman.Option<T>.Default());
    }
  ...
  public class Option_None<T> : Option<T> {
    ...
    public override bool Equals(object other) {
      var oth = other as Huffman.Option_None<T>;
      return oth != null;
    }
    public override int GetHashCode() {
      ulong hash = 5381;
  ...
  public class Option_Some<T> : Option<T> {
    ...
    public override bool Equals(object other) {
      var oth = other as Huffman.Option_Some<T>;
  ...
  public interface _IHTree<Sym> {
    bool is_Leaf { get; }
    bool is_Node { get; }
    Sym dtor_sym { get; }
    BigInteger dtor_w { get; }
    Huffman._IHTree<Sym> dtor_left { get; }
    Huffman._IHTree<Sym> dtor_right { get; }
    _IHTree<__Sym> DowncastClone<__Sym>(Func<Sym, __Sym> converter0);
  }
  public abstract class HTree<Sym> : _IHTree<Sym> {
    public HTree() {
    }
    public static Huffman._IHTree<Sym> Default(Sym _default_Sym) {
      return create_Leaf(_default_Sym, BigInteger.Zero);
    }
    public static Dafny.TypeDescriptor<Huffman._IHTree<Sym>> _TypeDescriptor(Dafny.TypeDescriptor<Sym> _td_Sym) {
      return new Dafny.TypeDescriptor<Huffman._IHTree<Sym>>(Huffman.HTree<Sym>.Default(_td_Sym.Default()));
    }
    public static _IHTree<Sym> create_Leaf(Sym sym, BigInteger w) {
      return new HTree_Leaf<Sym>(sym, w);
    }
    public static _IHTree<Sym> create_Node(BigInteger w, Huffman._IHTree<Sym> left, Huffman._IHTree<Sym> right) {
      return new HTree_Node<Sym>(w, left, right);
    }
    public bool is_Leaf { get { return this is HTree_Leaf<Sym>; } }
    public bool is_Node { get { return this is HTree_Node<Sym>; } }
    public Sym dtor_sym {
      get {
        var d = this;
        return ((HTree_Leaf<Sym>)d)._sym;
      }
    }
    public BigInteger dtor_w {
      get {
        var d = this;
        if (d is HTree_Leaf<Sym>) { return ((HTree_Leaf<Sym>)d)._w; }
        return ((HTree_Node<Sym>)d)._w;
      }
    }
    public Huffman._IHTree<Sym> dtor_left {
      get {
        var d = this;
        return ((HTree_Node<Sym>)d)._left;
      }
    }
    public Huffman._IHTree<Sym> dtor_right {
      get {
        var d = this;
        return ((HTree_Node<Sym>)d)._right;
      }
    }
    public abstract _IHTree<__Sym> DowncastClone<__Sym>(Func<Sym, __Sym> converter0);
  }
  public class HTree_Leaf<Sym> : HTree<Sym> {
    public readonly Sym _sym;
    public readonly BigInteger _w;
    public HTree_Leaf(Sym sym, BigInteger w) : base() {
      this._sym = sym;
      this._w = w;
    }
    public override _IHTree<__Sym> DowncastClone<__Sym>(Func<Sym, __Sym> converter0) {
      if (this is _IHTree<__Sym> dt) { return dt; }
      return new HTree_Leaf<__Sym>(converter0(_sym), _w);
    }
    public override bool Equals(object other) {
      var oth = other as Huffman.HTree_Leaf<Sym>;
      return oth != null && object.Equals(this._sym, oth._sym) && this._w == oth._w;
    }
    public override int GetHashCode() {
      ulong hash = 5381;
      hash = ((hash << 5) + hash) + 0;
      hash = ((hash << 5) + hash) + ((ulong)Dafny.Helpers.GetHashCode(this._sym));
      hash = ((hash << 5) + hash) + ((ulong)Dafny.Helpers.GetHashCode(this._w));
      return (int) hash;
    }
    public override string ToString() {
      string s = "Huffman.HTree.Leaf";
      s += "(";
      s += Dafny.Helpers.ToString(this._sym);
      s += ", ";
      s += Dafny.Helpers.ToString(this._w);
      s += ")";
      return s;
    }
  }
  public class HTree_Node<Sym> : HTree<Sym> {
    public readonly BigInteger _w;
    public readonly Huffman._IHTree<Sym> _left;
    public readonly Huffman._IHTree<Sym> _right;
    public HTree_Node(BigInteger w, Huffman._IHTree<Sym> left, Huffman._IHTree<Sym> right) : base() {
      this._w = w;
      this._left = left;
      this._right = right;
    }
    public override _IHTree<__Sym> DowncastClone<__Sym>(Func<Sym, __Sym> converter0) {
      if (this is _IHTree<__Sym> dt) { return dt; }
      return new HTree_Node<__Sym>(_w, (_left).DowncastClone<__Sym>(Dafny.Helpers.CastConverter<Sym, __Sym>), (_right).DowncastClone<__Sym>(Dafny.Helpers.CastConverter<Sym, __Sym>));
    }
    public override bool Equals(object other) {
      var oth = other as Huffman.HTree_Node<Sym>;
      return oth != null && this._w == oth._w && object.Equals(this._left, oth._left) && object.Equals(this._right, oth._right);
    }
    public override int GetHashCode() {
      ulong hash = 5381;
      hash = ((hash << 5) + hash) + 1;
...
namespace _module {
  ...
  public partial class __default {
    public static void __Test____Main__(Dafny.ISequence<Dafny.ISequence<Dafny.Rune>> __noArgsParameter)
    {
      bool _0_success;
      _0_success = true;
      Dafny.Helpers.Print((Dafny.Sequence<Dafny.Rune>.UnicodeFromString(@"Huffman.DemoRun: ")).ToVerbatimString(false));
      try {
        {
          Huffman.__default.DemoRun();
          {
...
")).ToVerbatimString(false));
      ...
      if (!(_0_success)) {
        throw new Dafny.HaltException("project.dfy(1,0): " + Dafny.Sequence<Dafny.Rune>.UnicodeFromString(@"Test failures occurred: see above.
").ToVerbatimString(false));}
    }
  }
